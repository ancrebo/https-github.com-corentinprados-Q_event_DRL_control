        ...
        if phi != 0:
            ...
            if case == "airfoil":
                if rotate_airfoil == 0:
                    drag_O = drag
                    lift_O = lift
                    drag = drag_O * np.cos(np.deg2rad(aoa)) + lift_O * np.sin(
                        np.deg2rad(aoa)
                    )
                    lift = lift_O * np.cos(np.deg2rad(aoa)) - drag_O * np.sin(
                        np.deg2rad(aoa)
                    )
...
def _timeSummation(
    ...
    if time < 0:
        ...
        for i in range(len(time_steps) - k, len(time_steps)):
            sum_drag += (
                array_drag[i] * time_steps[i]
            )  # el drag en cada instante de la sim y hace integral para luego la media
...
def compute_avg_lift_drag(ep_num: int, cpuid: str ="", nb_inv: int =-1, global_rew: bool =False) -> Tuple[float, float]:
    ...
    if ep_num == 0:
        ...
        entrada = open(
            "alya_files/{0}/1/EP_{1}/".format(cpuid, ep_num) + filename + file, "r"
        )
    ...
    time_steps, accumulated_array = _timeArrays(
        uniqueIndex, index, fileLines, totalBound
    )
    ...
    array_drag, array_lift, array_side, array_roll, array_pitch, array_yaw = (
        _calculateFnM(
            uniqueIndex,
            fileLines,
            bound,
