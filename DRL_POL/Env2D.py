...
from alya import write_physical_properties, write_time_interval, write_run_type
...
class Environment2D(Environment):
    ...
    def start_class(self):
        ...
        if self.continue_training:
            ...
            average_drag, average_lift = compute_avg_lift_drag(
                self.episode_number, cpuid=temp_id
            )
    ...
    def run(self, which):
        ...
        if which == "reset":
            ...
            filepath = os.path.join(
                "alya_files", "%s" % self.host, "EP_%d" % self.episode_number
            )
            write_time_interval(
                filepath, self.simulation_timeframe[0], self.simulation_timeframe[1]
            )
    ...
    def run_alya(self, which):
        ...
        if which == "reset":  ## only for the baseline_case
            ...
            run_subprocess(
                casepath, ALYA_SETS, "%s-boundary.nsi.set 3" % self.case, log=logssets
            )
    ...
    def switch(self, state):
        ...
        # Open file
        filepath = (
            os.path.join("alya_files", "baseline")
                ...
                if not os.path.exists("saved_models/" + name):
                    with open("saved_models/" + name, "w") as csv_file:
                        ...
                        spam_writer.writerow(
                            [self.last_episode_number, avg_drag, avg_lift]
                        )
                else:
    ...
    def save_this_action(self):
        ...
        if not os.path.exists(
            "actions/{}/ep_{}/".format(self.host, self.episode_number)
        ):
    ...
    def save_reward(self, reward):
        ...
        if not os.path.exists(
            "rewards/{}/ep_{}/".format(self.host, self.episode_number)
        ):
    ...
    def recover_start(self):
        ...
        runargs = "baseline %s" % os.path.join(
            "%s" % self.host, "EP_%d" % self.episode_number
        )
        ...
        if self.episode_number > 1:
            ...
            runargs = os.path.join(
                "%s" % self.host, "EP_%d" % (self.episode_number - 1)
            )
    ...
    def states(self):
        ...
        return dict(type="float", shape=(len(self.output_params["locations"]),))
    ...
    def reset(self):
        ...
        write_time_interval(
            os.path.join("alya_files", "%s" % self.host, "EP_%d" % self.episode_number),
            t1,
            t2,
        )
    ...
    def execute(self, actions):
        ...
        average_drag, average_lift = compute_avg_lift_drag(
            self.episode_number, cpuid=self.host
        )
        self.history_parameters["drag"].extend([average_drag])
        self.history_parameters["lift"].extend([average_lift])
        self.history_parameters["time"].extend([self.last_time])
