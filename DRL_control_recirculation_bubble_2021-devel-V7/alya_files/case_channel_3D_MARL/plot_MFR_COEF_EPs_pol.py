        ...
        if len(line) != 0:
            ...
            if line[0] == "time":
                ...
                if time > 0:
                    ...
                    superTitle = (
                        "Averaged from "
                        + str(abs(initial_time))
                        + " to "
                        + str(abs(initial_time + time))
                    )
                elif time < 0:
            ...
            if line[0] == "rotation_vector":
                ...
                u = npvro / (
                    math.sqrt(((npvro[0]) ** 2) + ((npvro[1]) ** 2) + ((npvro[2]) ** 2))
                )
            if line[0] == "phi":
                phi = float(line[2])
...
def _timeArrays(uniqueIndex, index):
    ...
    for i in range(len(uniqueIndex)):
        ...
        time_step = float(fileLines[line][3]) - float(
            fileLines[line - 2 - totalBound][3]
        )
...
def _calculateFnM(
    ...
    for i in range(len(uniqueIndex)):
        ...
        if len(M_visc_columns) != 0 or len(M_pres_columns) != 0:
            ...
            roll = (
                -RM * 2 / (float(density) * float(scale_area) * float(veloc) ** 2) * d
            )
            pitch = (
                -PM * 2 / (float(density) * float(scale_area) * float(veloc) ** 2) * d
            )
...
def _plots(
    ...
    plt.axhline(
        ...
        color="k",
        linewidth=2,
        label=r"$\overline{C_d}$",
    )
    plt.axhline(
        y=average_drag + rms_drag,
        xmin=0,
        xmax=1,
        linestyle="--",
        color="k",
        label=r"$\overline{C_d} \pm \sigma$",
    )
    ...
    plt.axhline(
        ...
        color="k",
        linewidth=2,
        label=r"$\overline{C_s}$",
    )
    plt.axhline(
        y=average_side + rms_side,
        xmin=0,
        xmax=1,
        linestyle="--",
        color="k",
        label=r"$\overline{C_s} \pm \sigma$",
    )
    ...
    np.savetxt("lift_ep.csv", plot_lift, fmt="%1.9d", delimiter=";")
    plt.axhline(
        y=average_lift,
        xmin=0,
        xmax=1,
        ...
        color="k",
        linewidth=2,
        label=r"$\overline{C_l}$",
    )
    plt.axhline(
        y=average_lift + rms_lift,
        xmin=0,
        xmax=1,
        linestyle="--",
        color="k",
        label=r"$\overline{C_l} \pm \sigma$",
    )
    ...
    if len(M_visc_columns) != 0 or len(M_pres_columns) != 0:
        ...
        plt.axhline(
            ...
            color="k",
            linewidth=2,
            label=r"$\overline{C_rm}$",
        )
        plt.axhline(
            y=average_roll + rms_roll,
            xmin=0,
            xmax=1,
            linestyle="--",
            color="k",
            label=r"$\overline{C_rm} \pm \sigma$",
        )
        plt.axhline(
            y=average_roll - rms_roll, xmin=0, xmax=1, linestyle="--", color="k"
        )
        plt.axhline(
            y=R_exp,
            xmin=0,
        ...
        plt.axhline(
            ...
            color="k",
            linewidth=2,
            label=r"$\overline{C_pm}$",
        )
        plt.axhline(
            y=average_pitch + rms_pitch,
            xmin=0,
            xmax=1,
            linestyle="--",
            color="k",
            label=r"$\overline{C_pm} \pm \sigma$",
        )
        plt.axhline(
            y=average_pitch - rms_pitch, xmin=0, xmax=1, linestyle="--", color="k"
        )
        plt.axhline(
            y=S_exp,
            xmin=0,
        ...
        plt.axhline(
            ...
            color="k",
            linewidth=2,
            label=r"$\overline{C_ym}$",
        )
        plt.axhline(
            y=average_yaw + rms_yaw,
            xmin=0,
            xmax=1,
            linestyle="--",
            color="k",
            label=r"$\overline{C_ym} \pm \sigma$",
        )
...
def _plotMass(accumulated_array, array_mass, plot_time):
    ...
    np.savetxt("mfr_top_ep.csv", plot_mass_top, fmt="%1.9d", delimiter=";")
    np.savetxt("mfr_bot_ep.csv", plot_mass_bot, fmt="%1.9d", delimiter=";")
    plt.figure(4)
...
array_drag, array_lift, array_side, array_roll, array_pitch, array_yaw = _calculateFnM(
    uniqueIndex,
    fileLines,
    bound,
    variabound,
