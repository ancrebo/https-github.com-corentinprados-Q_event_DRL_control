...
def _avNrms(sum_drag, sum_side, sum_lift, sum_roll, sum_pitch, sum_yaw, tu, M_visc_columns, M_pres_columns):
	...
	average_effi = average_lift/average_drag
	if len(M_visc_columns) != 0 or len(M_pres_columns) != 0:
	...
	for i in range(initial_step, final_step):
		...
		sum_effi_rms  += (array_lift[i]/array_drag[i]-average_effi)**2
		if len(M_visc_columns) != 0 or len(M_pres_columns) != 0:
	...
	if len(M_visc_columns) != 0 or len(M_pres_columns) != 0:
		return(average_drag, average_side, average_lift, average_roll, average_pitch, average_yaw,
			rms_drag, rms_side, rms_lift, rms_effi, rms_roll, rms_pitch, rms_yaw)
	else:
		return(average_drag, average_side, average_lift, average_effi, False, False, False,
			rms_drag, rms_side, rms_lift, rms_effi, False, False, False)

# Print coefficients
def _printCoeff(average_drag, average_side, average_lift, average_effi, average_roll, average_pitch, average_yaw, rms_drag, rms_side,
	rms_lift, rms_effi, rms_roll, rms_pitch, rms_yaw, M_visc_columns, M_pres_columns):
	...
	print('Average_effi: ' + str(average_effi))
	print('RMS_effi: ' + str(rms_effi))
	if len(M_visc_columns) != 0 or len(M_pres_columns) != 0:
...
def _plotArrays(accumulated_array, array_drag, array_side, array_lift, initial_step, final_step, M_visc_columns, M_pres_columns):
	...
	for i in range(initial_step, final_step):
		...
		plot_effi.append(array_lift[i]/array_drag[i])
	...
	if len(M_visc_columns) != 0 or len(M_pres_columns) != 0:
		...
		return (plot_drag, plot_lift, plot_side, plot_effi, plot_time, False, False, False, av_initial_time, av_final_time)

# Plot
def _plots(superTitle, plot_time, av_initial_time, av_final_time, plot_drag, average_drag, rms_drag,
	plot_side, average_side, rms_side, plot_lift, average_lift, rms_lift, plot_effi, average_effi, rms_effi, plot_roll, average_roll, rms_roll,
	...
	plt.ylim([average_drag - 6*rms_drag, average_drag + 6*rms_drag])
	...
	plt.grid(True, which='major', axis='both', linestyle=':')
	plt.ylim([average_side - 6*rms_side, average_side + 6*rms_side])
	...
	plt.grid(True, which='major', axis='both', linestyle=':')
	plt.ylim([average_lift - 6*rms_lift, average_lift + 6*rms_lift])
    
	plt.figure(4)
	plt.suptitle(superTitle, fontsize=20)
    
	plt.plot(plot_time, plot_effi,'r-', label=r"$C_l$" )
	plt.axhline(y=average_effi, xmin=0, xmax=1, linestyle='-.', color = 'k', linewidth=2, label=r"$\overline{C_l}$")
	plt.axhline(y=average_effi + rms_effi, xmin=0, xmax=1, linestyle='--', color = 'k', label=r"$\overline{C_l} \pm \sigma$")
	plt.axhline(y=average_effi - rms_effi, xmin=0, xmax=1, linestyle='--', color = 'k')
	plt.axvspan(av_initial_time, av_final_time, facecolor='0.5', alpha=0.2)
	plt.legend(loc='lower left', fancybox=True)
	plt.title('Aerodynamic efficiency', fontsize=15)
	plt.xlabel('Time (s)', fontsize=12)
	plt.ylabel(r"$C_l/C_d$", fontsize=12)
	plt.grid(True, which='major', axis='both', linestyle=':')
	plt.ylim([average_effi - 6*rms_effi, average_effi + 6*rms_effi])
...
average_drag, average_side, average_lift, average_effi, average_roll, average_pitch, average_yaw, rms_drag, rms_side, rms_lift, rms_effi, rms_roll, rms_pitch, rms_yaw = _avNrms(sum_drag, sum_side, sum_lift, sum_roll, sum_pitch, sum_yaw, tu, M_visc_columns, M_pres_columns)

_printCoeff(average_drag, average_side, average_lift, average_effi, average_roll, average_pitch, average_yaw, rms_drag, rms_side,
	rms_lift, rms_effi, rms_roll, rms_pitch, rms_yaw, M_visc_columns, M_pres_columns)

plot_drag, plot_lift, plot_side, plot_effi, plot_time, plot_roll, plot_pitch, plot_yaw, av_initial_time, av_final_time = _plotArrays(accumulated_array, array_drag, array_side, array_lift, initial_step, final_step, M_visc_columns, M_pres_columns)
