...
axs[2,0].plot(5*(np.abs(data['AvgDrag']-cd_base)-0.6*data['AvgLift']), color ='k', linewidth=0.5, alpha = 0.5)
axs[2,0].plot(range(12,len(data['AvgDrag'])-2), np.convolve(5*(np.abs(data['AvgDrag']-cd_base)-0.6*data['AvgLift']),np.ones(15), 'valid' )/15, 'k', linewidth=2)
#axs[0,0].set_title("Local Cd during (Re = %s)" %num_re, fontsize=size_title)
axs[2,0].set_xlabel("Pseudoenvironments episodes", fontsize=size_title)
axs[2,0].set_ylabel("Local Reward", fontsize=size_title)
axs[2,0].tick_params(axis='both',labelsize=size_title)
axs[2,0].axhline(y=0, color='k', linestyle='--', label='Baseline r = 0', linewidth=linewidth_legend)
#axs[0,0].axhline(y=cd_2D, color='m', linestyle='--',   label='2D training        Cd = %0.2f' %cd_2D, linewidth=linewidth_legend)
axs[2,0].axhline(y=1, color='r', linestyle='--',label='Maximum reward r = 1', linewidth=linewidth_legend)
axs[2,0].legend(loc='best', fancybox=True)
axs[2,0].grid()
#axs[0,0].gcf().suptitle('fig1')
#axs[0,0].tight_layout()
#axs[0,0].show()
plt.savefig("rew_plot_re%s_3D_MARL.png" %num_re)


axs[2,1].plot(5*(np.abs(data['AvgDrag_GLOBAL']-cd_base)-0.6*data['AvgLift_GLOBAL']), color ='k', linewidth=0.5, alpha = 0.5)
axs[2,1].plot(range(12,len(data['AvgDrag'])-2), np.convolve(5*(np.abs(data['AvgDrag_GLOBAL']-cd_base)-0.6*data['AvgLift']),np.ones(15), 'valid' )/15, 'k', linewidth=2)
#axs[0,0].set_title("Local Cd during (Re = %s)" %num_re, fontsize=size_title)
axs[2,1].set_xlabel("Pseudoenvironments episodes", fontsize=size_title)
axs[2,1].set_ylabel("Global Reward", fontsize=size_title)
axs[2,1].tick_params(axis='both',labelsize=size_title)
axs[2,1].axhline(y=0, color='k', linestyle='--', label='Baseline r = 0', linewidth=linewidth_legend)
#axs[0,0].axhline(y=cd_2D, color='m', linestyle='--',   label='2D training        Cd = %0.2f' %cd_2D, linewidth=linewidth_legend)
axs[2,1].axhline(y=1, color='r', linestyle='--',label='Maximum reward r = 1', linewidth=linewidth_legend)
axs[2,1].legend(loc='best', fancybox=True)
axs[2,1].grid()
#axs[0,0].gcf().suptitle('fig1')
#axs[0,0].tight_layout()
#axs[0,0].show()
plt.savefig("rewglob_plot_re%s_3D_MARL.png" %num_re)

axs[1,0].plot(data['AvgLift'], color ='k', linewidth=0.5, alpha = 0.5)
axs[1,0].plot(range(12,len(data['AvgDrag'])-2), np.convolve(data['AvgLift'],np.ones(15), 'valid' )/15, 'r', linewidth=2)
#axs[0,1].set_title("Local Cl training (Re = %s)" %num_re, fontsize=size_title)
axs[1,0].set_xlabel("Pseudoenvironments episodes", fontsize=size_title)
axs[1,0].set_ylabel("Cl local", fontsize=size_title)
axs[1,0].tick_params(axis='both',labelsize=size_title)
axs[1,0].axhline(y=0, color='k', linestyle='--',        label='Baseline           Cl = %0.2f' %cl_base, linewidth=linewidth_legend)
#axs[0,1].axhline(y=0, color='m', linestyle='--',        label='2D training        Cl = %0.2f' %cl_2D, linewidth=linewidth_legend)
axs[1,0].axhline(y=cl_deter, color='r', linestyle='--', label='3D Deterministic Cl = %0.2f' %cl_deter, linewidth=linewidth_legend)
...
axs[1,1].plot(np.arange(8*0.1,len(data['AvgDrag_GLOBAL'])*0.1-0.1,0.1), np.convolve(data['AvgLift_GLOBAL'],np.ones(10), 'valid' )/10, 'c', linewidth=2)
...
axs[0,1].plot(np.arange(8*0.1,len(data['AvgDrag_GLOBAL'])*0.1-0.1,0.1), np.convolve(data['AvgDrag_GLOBAL'],np.ones(10), 'valid' )/10, 'm', linewidth=2)
#axs[1,0].set_title("Global Cd training (Re = %s)" %num_re, fontsize=size_title)
axs[0,1].set_xlabel("CFD runs", fontsize=size_title)
axs[0,1].set_ylabel("Cd global", fontsize=size_title)
axs[0,1].tick_params(axis='both',labelsize=size_title)
axs[0,1].axhline(y=cd_base, color='k', linestyle='--', label='Baseline           Cd = %0.2f' %cd_base, linewidth=linewidth_legend)
#axs[1,0].axhline(y=cd_2D, color='m', linestyle='--',   label='2D training        Cd = %0.2f' %cd_2D, linewidth=linewidth_legend)
axs[0,1].axhline(y=cd_deter, color='r', linestyle='--',label='3D Deterministic Cd = %0.2f' %cd_deter, linewidth=linewidth_legend)
