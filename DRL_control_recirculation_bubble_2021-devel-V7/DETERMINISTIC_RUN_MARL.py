...
run_subprocess(
    "./", "cp", "parameters/parameters_{}.py parameters.py".format(training_case)
)
...
environment_base = Environment(
    simu_name=simu_name, node=nodelist[1], deterministic=True
)
...
def list_observation(probes_values_global, ENV_ID, nb_inv_per_CFD):
    ...
    if not neighbor_state:
        ...
        probes_values_2 = probes_values_global[
            ((ENV_ID[1] - 1) * batch_size_probes) : (ENV_ID[1] * batch_size_probes)
        ]
        ...
        if ENV_ID[1] == 1:
            probes_values_halo = probes_values_global[
                ((nb_inv_per_CFD - 1) * batch_size_probes) : (
                    nb_inv_per_CFD * batch_size_probes
                )
            ]
            probes_values = probes_values_global[
                ((ENV_ID[1] - 1) * batch_size_probes) : (
                    (ENV_ID[1] + 1) * batch_size_probes
                )
            ]
            ...
            probes_values = probes_values_global[
                ((ENV_ID[1] - 2) * batch_size_probes) : (ENV_ID[1] * batch_size_probes)
            ]
...
for action_step in range(nb_actuations_deterministic):
    for e in reversed(environments):
        ...
        list_actions = [
            agent.act(
                list_states[e.ENV_ID[1] - 1], deterministic=True, independent=True
            )
        ]
    ...
    for e in environments:
        if e.ENV_ID[1] != 1:
            ...
            filename = os.path.join(
                "alya_files", "environment1", "1", "EP_1", "cylinder.nsi.wit"
            )

            # read witness file and extract the entire array list
            probes_values_global = read_last_wit(
                filename,
                output_params["probe_type"],
                optimization_params["norm_press"],
                nwit_to_read,
            )

            # filter probes per jet (corresponding to the ENV.ID[])
            list_states[e.ENV_ID[1] - 1] = list_observation(
                nb_inv_per_CFD=nb_inv_per_CFD,
                probes_values_global=probes_values_global,
                ENV_ID=e.ENV_ID,
            )
